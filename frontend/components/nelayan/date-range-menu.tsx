...
type DateRangeMenuProps = {
  action: string
  // The rest of the view (status, order), carried through the form so applying a range keeps it.
  keep: Record<string, string>
  range: DateRange
  // What the button reads, and its accessible name ("Ubah rentang tanggal: …").
  value: string
  editLabel: string
  copy: DateRangeMenuCopy
}
...
// land in the URL as ?dari= / ?sampai=. The export only draws the closed control; the panel follows the marketplace
// filter panels.
export function DateRangeMenu({ action, keep, range, value, editLabel, copy }: DateRangeMenuProps) {
    ...
    <div ref={rootRef} className="box-border w-[280px] shrink-0 h-fit flex flex-col gap-[6px] justify-start items-start relative">
      ...
      >
        ...
        <fieldset className="box-border w-full flex flex-col gap-[10px] m-0 p-0 border-0 min-w-0">
          <legend className="text-[12px]/[normal] box-border text-[#5B6B7C] font-inter font-normal text-left p-[0px_0px_4px_0px]">{copy.legend}</legend>
          <label className="box-border w-full flex flex-col gap-[4px] justify-start items-start">
            <span className="text-[12px]/[normal] box-border text-[#5B6B7C] font-inter font-medium text-left">{copy.fromLabel}</span>
          ...
          <label className="box-border w-full flex flex-col gap-[4px] justify-start items-start">
            <span className="text-[12px]/[normal] box-border text-[#5B6B7C] font-inter font-medium text-left">{copy.toLabel}</span>
            <span className={`${CONTROL} ${FOCUS_WITHIN}`}>
