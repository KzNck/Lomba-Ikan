...
const BACK_CLASS = `box-border w-fit shrink-0 h-fit flex flex-row gap-[12px] p-[13px_28px] justify-center items-center bg-[#FFFFFF] [outline:1.5px_solid_#0F6CB8] [outline-offset:-0.75px] rounded-[999px] cursor-pointer ${OUTLINE_HOVER} ${PRESS} ${FOCUS_RING}`

// The modal's "Footer": a hairline divider, the outline back link and the gradient submit button.
// Render it inside the step's <form> so the button submits it.
export function CatchFooter({ back, submitLabel }: CatchFooterProps) {
  ...
  return (
    <div className="[box-sizing:content-box] w-full md:w-[656px] h-fit shrink-0 flex flex-row gap-[12px] md:gap-0 [&>*]:flex-1 md:[&>*]:[flex:0_1_auto] p-[16px_0px_0px_0px] justify-between items-center [border-width:1px_0px_0px_0px] [border-style:solid] [border-color:#E2E8F0] [margin:-0.5px_0px_0px_0px]">
      {back.href !== undefined ? (
