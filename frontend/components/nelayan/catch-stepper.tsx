...
// The "Stepper Tangkapan" component. Fixed 72px steps with connectors that stretch to fill the modal width.
const STEP_STATES = {
  // Filled circle with a check instead of the number.
  completed: {
    circle: 'bg-[#0F6CB8]',
    label: 'text-[#0B3B5C] font-medium',
  },
  active: {
    circle: 'bg-[#0F6CB8]',
    number: 'text-[#FFFFFF]',
...
type CatchStepperProps = {
  steps: string[]
  // Zero-based index of the step the user is on. Earlier steps render as completed.
  currentStep: number
}

export function CatchStepper({ steps, currentStep }: CatchStepperProps) {
  return (
    <ol className="box-border w-full h-fit shrink-0 flex flex-row gap-0 justify-start items-start">
      {steps.map((label, index) => {
        const status = index < currentStep ? 'completed' : index === currentStep ? 'active' : 'upcoming'
        const state = STEP_STATES[status]

        return (
          // Each item after the first carries the connector before it and takes its flexible width, so the row lays
          // out exactly like the export's flat step/connector list.
          <li
            key={label}
            className={`box-border ${index > 0 ? '[flex:1_1_0]' : 'w-fit shrink-0'} h-fit flex flex-row gap-0 justify-start items-start`}
          >
            {index > 0 && (
              <div
                aria-hidden="true"
                className="box-border [flex:1_1_0] h-fit flex flex-row gap-0 p-[15px_4px_0px_4px] justify-start items-start"
              >
                {/* The connector turns blue once the step it leads into has been reached. */}
                <div className={`box-border [flex:1_1_0] h-[2px] ${index <= currentStep ? 'bg-[#0F6CB8]' : 'bg-[#C5DDF0]'} rounded-[1px]`} />
              </div>
            )}
            <div
