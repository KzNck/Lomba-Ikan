...
// The account menu behind the header pill and the sidebar user card: "Akun", then "Keluar" under a divider.
...
export function AccountMenu({ name, accountHref, placement, triggerClassName, chevronFill, children }: AccountMenuProps) {
  ...
  return (
    <div ref={rootRef} className="box-border w-full h-fit relative">
      <button {...buttonProps} aria-haspopup="menu" aria-label={ACCOUNT_MENU.triggerLabel(name)} className={triggerClassName}>
        {children}
        <Icon
          name="chevron-down"
          fill={chevronFill}
