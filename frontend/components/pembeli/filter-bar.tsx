...
export function FilterBar({ label, filters, reset }: FilterBarProps) {
  return (
    <div role="group" aria-label={label} className="box-border w-full h-fit shrink-0 flex flex-row gap-[12px] justify-start items-center">
...
export function ResetFilterLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      scroll={false}
      className={`box-border w-fit shrink-0 h-[40px] flex flex-row gap-[6px] p-[0px_14px] justify-start items-center bg-[#FFFFFF] [outline:1px_solid_#E2E8F0] [outline-offset:-0.5px] rounded-[999px] ${OUTLINE_HOVER} ${PRESS} ${FOCUS_RING}`}
    >
